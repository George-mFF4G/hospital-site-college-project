...
<div class="container-form">
  <form method="POST" action="{{Route('vaccaine.store')}}">      
    @csrf
    <div class="title">
        <i class="fas fa-pencil-alt"></i>
        <h2>Enter Vaccaine Information</h2>
    </div>
    <div>            
            <input type="text" name="patient_name" class="feedback-input" placeholder="Patient Name" value="{{old('patient_name')}}">
            @error('patient_name')
            ...
            <div class="alert alert-danger">{{$message}}</div>
            @enderror
            <input type="text" name="description" class="feedback-input" placeholder="Description" value="{{old('description')}}">
            @error('description')
            <div class="alert alert-danger">{{$message}}</div>
            @enderror
            <label style="color:#fff; font-weight:500;">Select Date</label>
			      <input class="feedback-input" name="appoint_date" type="date" required>
            @error('appoint_date')
            <div class="alert alert-danger">{{$message}}</div>
            @enderror
    </div>
            <input type="submit" name="submit" value="Add New Vaccaine"> 
        </div>  
  </form>
</div>
